...
| R-10 | Large results exhaust memory or freeze UI | M | Critical | 1M/10M row, slow consumer, hostile length, RSS/frame metrics | Pull streaming, byte+row caps, bounded queues/pages, virtualization | Stop fetch/export stream; disable feature at lower limit | Performance + core owner |
...
| R-23 | Local history/diagnostics leak secrets or sensitive SQL/data | M | High | Seeded canary scans of SQLite/log/crash/export/clipboard/temp | Redaction schema before sinks, opt-in network, retention/delete controls | Purge artifacts, rotate affected credentials, disable diagnostics | Security/privacy owner |
